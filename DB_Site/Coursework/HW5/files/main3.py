...
def main():
    ...
    with open("../../infile.csv", mode = 'r') as source_file:
        ...
        #open target file
        with open("outfile3.csv", mode = 'w') as target_file:
            ...
            header = ["User","White","User","Black","[:]","Victory","Winner","+/-","-","ECO","System"]
            ...
            for line in sorted_final:
                ...
                if header == False:
                    ...
                    gameTime = line[7]
                    whiteID = line[8]
                    whiteRating = int(line[9])
                    blackID = line[10]
                    blackRating = int(line[11])
                    if whiteRating > blackRating:
                        underdog = "b"
                    elif whiteRating < blackRating:
                        underdog = "w"
                    else:
                        underdog = " "
                    winner = line[6]
                    difference = abs(whiteRating-blackRating)
                    moves = line[4]
                    victory_type = line[5]
                    if victory_type == "outoftime":
                        victory_type = "time";
                    eco = line[13]
                    untilPosition = int(line[15])
                    system = line[14]
                    #optional:
                    game_log = line[12]
                    ...
                    if rated == "TRUE":
                        ...
                        if response == "yes":
                            ...
                            csv_writer.writerow((gameTime,whiteID,whiteRating,blackID,blackRating,moves,victory_type,winner,underdog,difference,eco,untilPosition,system,game_log))
                            #write the line
                            lines_written +=1
                            ...
                            csv_writer.writerow((gameTime,whiteID,whiteRating,blackID,blackRating,moves,victory_type,winner,underdog,difference,eco,untilPosition,system))
